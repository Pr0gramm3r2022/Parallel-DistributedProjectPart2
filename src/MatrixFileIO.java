...
public class MatrixFileIO {
    ...
    public static void main(String[] args) {
            ...
            // Read matrices back
            for(int matrixListSize = 2; matrixListSize <= 64; matrixListSize*=2){
                for(int matrixDimension = 2; matrixDimension <= 200; matrixDimension*=2){
                    List<double[][]> readMatrices = MatrixGenerator2.generateMatrices(matrixListSize,matrixDimension);          

                    System.out.println();
                    BinaryTree tree = new BinaryTree(readMatrices);
                    long normTime = tree.treeMultiplication();
                    System.out.println("Result of Strassen's multiplication:");
                // printMatrix(tree.root.nodeMatrix);
                    System.out.println();
                    tree = new BinaryTree(readMatrices);
                    long multiTime = tree.treeMultiplicationMultithreaded();    
                    System.out.println("Result of Strassen's multiplication with multithreading:");
                // printMatrix(tree.root.nodeMatrix);
                    
                    System.out.println();
                    System.out.println("Matrix Length: " + readMatrices.get(0).length);
                    System.out.println("Number of Matrices: " + readMatrices.size());
                    System.out.println("Speedup: " + (double)normTime / multiTime);
                    System.out.println("Efficiency: " + (double)normTime / multiTime / Runtime.getRuntime().availableProcessors());
                }
            }
